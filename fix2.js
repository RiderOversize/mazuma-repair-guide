...
const startIdx = content.indexOf('export async function getTopModels');
if (startIdx !== -1) {
  const before = content.substring(0, startIdx);
  const newBottom = \export async function getTopModels(limit: number = 5): Promise<{modelId: string, count: number}[]> {
...
\;
  fs.writeFileSync('lib/sheets-db.ts', before + newBottom);
